...
typedef NS_ENUM(NSInteger, FFImagePositionType) {
    FFImagePositionTypeLeft,   //图片在左，标题在右，默认风格
    FFImagePositionTypeRight,  //图片在右，标题在左
    FFImagePositionTypeTop,    //图片在上，标题在下
    FFImagePositionTypeBottom  //图片在下，标题在上
};

typedef NS_ENUM(NSInteger, FFEdgeInsetsType) {
    FFEdgeInsetsTypeTitle,//标题
    FFEdgeInsetsTypeImage//图片
};

typedef NS_ENUM(NSInteger, FFMarginType) {
    FFMarginTypeTop         ,
    FFMarginTypeBottom      ,
    FFMarginTypeLeft        ,
    FFMarginTypeRight       ,
    FFMarginTypeTopLeft     ,
    FFMarginTypeTopRight    ,
    FFMarginTypeBottomLeft  ,
    FFMarginTypeBottomRight
};
...
@interface UIButton (FFEdgeInsets)
